...
1.  **Initial Hypothesis (Incorrect):** It was initially assumed that the CSP was being configured in `next.config.js` and that the issue was a missing directive there.
    - **Mistake:** The `next.config.js` file _did_ contain the correct CSP directives, but they were not being applied in production. This led to a brief period of confusion.
