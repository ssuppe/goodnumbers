...
- **Risk**: **Data Privacy Leak**. Sending health data to a public LLM.
  - _Mitigation_: Use Vertex AI Client. Sanitize input prompts (remove user metadata).
- **Risk**: **Hallucination / Medical Advice**.
  - _Mitigation_: UI Disclaimer. System Prompt engineering ("You are an analyst, not a doctor").
- **Risk**: **Model Availability**. `gemini-3.0` might be unavailable.
  - _Mitigation_: Configurable `AI_MODEL_NAME` env var. Fallback to `gemini-2.0-flash-exp`.
...
- _Privacy Note_: Add a comment block explaining how to swap this file for Vertex AI.
