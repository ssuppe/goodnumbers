...
def _generate_clinical_report(assessment, daily_stats):
    ...
    if assessment['composite_has_dawn']: 
        ...
        recommendations = add_comment("When looking at the average of all days in the past week, we do see patterns of dawn phenomenon.", recommendations)
        recommendations = add_comment(
            """Dawn phenomenon is when your blood sugar rises more than it should
               ...
               means they need to provide a higher basal rate to compensate.""", recommendations)

        if assessment["severity"]["is_severe"]:
            recommendations = add_comment(
            f"""We are seeing quite a severe rise in blood
                sugars during this time - raising more than {SIGNIFICANT_RISE} mg/dl
                at more than {SIGNIFICANT_RATE} mg/dl per hour.""", recommendations)
            
            recommendations = add_comment(
                f"""A deeper look:
                            * Average morning glucose at this time is 
                                {report['metrics']['average_morning_glucose']}. 
                                {assessment["percent_days_with_dawn"]} days showed dawn phenomena.
                            * Average coefficient of variation 
                            {report['metrics']['average_cv']}%. The widely 
                            accepted goal for type 1 diabetics is to be below 36%.""", recommendations)
            
            if assessment['severity']['variability_concern']:
                recommendations = add_comment("High glucose variability detected. Consider basal rate adjustment.", recommendations)
        

            
        recommendations = add_comment(f"""* Time in range in the morning is 
                                        {report['metrics']['average_time_in_range']}. Your sleep time is when you should 
                                        be aim to get the highest time in range, since you are not moving, not worrying about 
                                        food, exercise or stress. Aim to be as close to 100% as possible.""", recommendations)
        
        if report["metrics"]["average_time_in_range"] < .80:
            recommendations = add_comment("""Your average time in range is less than 80% this week, so you could definitely improve.""", recommendations)
        elif report["metrics"]["average_time_in_range"] > .90:
            recommendations = add_comment("""Your average time in range is greater than 90% this week, this is very good!""", recommendations)
        else:
            recommendations = add_comment("""Your average time in range was between 80% and 90% this week, which is good,
                                          but there is always room for improvement!""", recommendations)
            
    
        if assessment['mean_metrics']['mean_first_hour_rate'] > 15:
            recommendations = add_comment("Significant early morning rise detected. Consider adjusting basal rates between 3-4 AM.", recommendations)
        ...
        recommendations = add_comment("""There is no sign of dawn phenomenon, which means your blood 
                                      sugar levels are staying within range and not rising before you wake up""", recommendations)
        recommendations = add_comment("""This is great! Let's look a bit deeper to see how you're 
                                      doing and see if there is any more room for improvement""", recommendations)
            
        recommendations = add_comment(f"""A deeper look:
                                    * Average morning glucose at this time is {report['metrics']['average_morning_glucose']}. 
                                    {assessment["percent_days_with_dawn"]} days showed dawn phenomena.
                                    * Average coefficient of variation {report['metrics']['average_cv']}%. The widely accepted goal for type 1 diabetics
                                    is to be below 36%.""", recommendations)
            
        if assessment['severity']['variability_concern']:
            recommendations = add_comment("High glucose variability detected. Consider basal rate adjustment.", recommendations)
            
        recommendations = add_comment(f"""* Time in range in the morning is {report['metrics']['average_time_in_range']}. Your sleep time is when you should 
                                        be aim to get the highest time in range, since you are not moving, not worrying about food, exercise or stress. Aim to be
                                        as close to 100% as possible.""", recommendations)
