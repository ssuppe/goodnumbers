...
export function ClusterEventsChart({
  ...
  const options = useMemo(() => {
    try {
      ...
      if (sortedEvents.length === 0) {
        console.warn(
          "[ClusterEventsChart] No valid events with >= 2 readings.",
        );
        return null;
      }

      if (process.env.NODE_ENV === "development") {
        console.log("[ClusterEventsChart] Preparing options:", {
        ...
        return {
          ...
          data: event.readings
            .map((r) => {
              ...
              if (isNaN(originalReadingTime) || isNaN(glucoseValue))
                return null;
            ...
            .filter(
              (d): d is { value: [number, number]; originalDate: string } =>
                Boolean(d),
            ),
        };
      });
      ...
      const finalMin = domain
        ? evidenceWindowMins > 0
          ? Math.min(
              domain.min,
              globalMinNormalized - evidenceWindowMins * 60000,
            )
          : domain.min
        : undefined;
  ...
  if (!options) {
    return (
      <div className="flex items-center justify-center h-64 bg-slate-50 border border-slate-100 rounded-lg text-slate-400 italic">
        Unable to generate visualization for this cluster.
      </div>
    );
  }
