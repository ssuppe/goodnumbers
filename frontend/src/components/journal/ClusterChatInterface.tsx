...
const ClusterChatInterface: React.FC<ClusterChatInterfaceProps> = ({
  ...
  return (
    <div className="relative border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm transition-all duration-300">
      {/* Chat Header */}
      <div className="bg-gray-50 border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        ...
        <button
          ...
          title="Exit chat and return to manual entry"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {/* Messages Pane */}
      <div className="flex flex-col space-y-3 p-4 h-[300px] overflow-y-auto bg-gray-50/30">
        {messages.map((m, idx) => (
          <div
            key={idx}
      ...
      >
        ...
        <button
          ...
          className="p-2 bg-mesa-primary text-white rounded-lg hover:bg-primary-hover transition-colors disabled:opacity-30 disabled:hover:bg-mesa-primary flex-shrink-0"
        >
      ...
      <div className="bg-gray-50 border-t border-gray-100 px-4 py-2 flex justify-end">
        <button
          ...
          type="button"
          disabled={isSending || isSynthesizing}
          className="px-4 py-1.5 bg-mesa-primary hover:bg-primary-hover text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-colors shadow-sm disabled:opacity-50"
        >
          Save as Insight
        </button>
      </div>
