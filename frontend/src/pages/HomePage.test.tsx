...
describe('HomePage', () => {
  beforeEach(() => {
    ...
    mockedUseAuth.mockReturnValue({ user: null, isLoading: false, error: null }); // Default unauthenticated state
    mockNavigate.mockClear();
  });

  it('renders marketing content for unauthenticated users', () => {
    render(<MemoryRouter><HomePage /></MemoryRouter>);
    expect(screen.getByRole('heading', { name: /A smart weekly journal for type 1 diabetics/i })).toBeInTheDocument();
  ...
  it('shows loading state when auth context is loading', () => {
    ...
    expect(screen.queryByRole('heading', { name: /A smart weekly journal for type 1 diabetics/i })).not.toBeInTheDocument();
  });

  it('redirects authenticated users to the dashboard', () => {
    const mockUser: SessionUser = { id: '1', email: 'test@example.com', name: 'Test User', agreementsSigned: true, nightscoutUrl: 'http://ns.com', preferredUnits: 'MGDL' };
