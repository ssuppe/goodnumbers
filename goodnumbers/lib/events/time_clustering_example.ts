...
function generateClusterDescription(cluster: TimeCluster): string {
  ...
  const eventTypeStr = cluster.eventType === GlycemicEventType.HYPOGLYCEMIA ? 
    'low blood sugar' : 'high blood sugar';
  
  // Check if it's a recurring pattern (more than one event)
  if (cluster.count > 1) {
    return `You had ${cluster.count} ${eventTypeStr} events around ${meanTimeStr} ` +
...
function runExample(): void {
  ...
  timeClusters.forEach((cluster, index) => {
    ...
    cluster.events.forEach(event => {
      ...
      console.log(`  - ${timeStr} on ${new Date(event.start_timestamp).toISOString().substr(0, 10)}, ` +
