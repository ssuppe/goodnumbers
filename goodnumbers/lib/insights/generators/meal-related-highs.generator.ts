...
import { ClassifiedEvent, EventClassificationType, DEFAULT_CLASSIFICATION_CONFIG } from '../../events/classification/classification_types';
 ...
 */
export function createMealRelatedHighsInsight(
  ...
  return {
    ...
    /**
     * Returns all generated insights (summary and specific classification insights)
     * This allows the caller to access the full set of insights beyond just the primary ones
     */
    getAllInsights(): { ai_insights: AssessmentInsight[], user_insights: AssessmentInsight[] } {
      return {
        ai_insights: analysisResults.aiInsights,
        user_insights: analysisResults.userInsights
      };
    }
  };
}
 ...
 */
function analyzeMealRelatedHighs(
  clusters: TimeCluster[]
): { aiInsights: AssessmentInsight[], userInsights: AssessmentInsight[] } {
  try {
    ...
    if (totalMealRelatedHighs === 0) {
      ...
      userInsights.push({
        note: `About ${percentMealRelated}% of your high glucose readings appear to be related to meals. Let's look at how we can improve your mealtime management to reduce these spikes.`,
        priority: InsightPriority.IMPORTANT
      });
    }
