...
- **Parameter Validation:** The `:id` route parameter used in `GET`, `PUT`, and `DELETE` must be validated to ensure it is a CUID. Requests with a malformed ID should be rejected with a `400 Bad Request` status *before* attempting a database query.
...
router.get("/:id", validateIdParam, async (req, res) => {
  ...
  try {
    ...
    console.error(`[API] Failed to fetch journal ${id} for user ${userId}:`, error);
...
router.put("/:id", validateIdParam, async (req, res) => {
  ...
  try {
    ...
    console.error(`[API] Failed to update journal ${id} for user ${userId}:`, error);
...
router.delete("/:id", validateIdParam, async (req, res) => {
  ...
  try {
    ...
    console.error(`[API] Failed to delete journal ${id} for user ${userId}:`, error);
