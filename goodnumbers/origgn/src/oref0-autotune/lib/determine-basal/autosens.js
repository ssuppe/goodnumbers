...
function detectSensitivity(inputs) {
  ...
  avgDeltas.sort(function (a, b) {
    return a - b;
  });
  bgis.sort(function (a, b) {
    return a - b;
  });
  ...
  var pSensitive = percentile(deviations, 0.5);
  var pResistant = percentile(deviations, 0.5);

  var average = deviationSum / deviations.length;
  //console.error("Mean deviation: "+average.toFixed(2));

  var squareDeviations = deviations.reduce(function (acc, dev) {
    var dev_f = parseFloat(dev);
...
function tempTargetRunning(temptargets_data, time) {
  ...
  try {
    temptargets_data.sort(function (a, b) {
