...
*   **Risk**: Stored XSS via malicious strings in Nightscout data.
    *   *Mitigation*: Zod `refine` check to reject HTML characters; React default escaping.
*   **Risk**: DoS via `O(N*M)` complexity in cluster analysis.
    *   *Mitigation*: Enforce hard limit on treatments (10k) and pre-sort by date.
*   **Risk**: JSON "Blindness" (Schema drift).
    *   *Mitigation*: Validate all JSON writes and reads against `InsightArraySchema`.
...
*   **Performance**: Sort treatments by date *once* before iterating clusters.
...
*   **Code (Schemas)**:
    ...
    export const InsightSchema = z.object({
      ...
      note: z.string().max(500).refine(val => !/[<>]/.test(val), {
...
*   **Code**:
    ...
    describe('Cluster Insights', () => {
      ...
      it('detects uncovered meal (Carbs > 0, Insulin = 0)', () => {
        ...
        const insights = generateClusterInsights(mockCluster as any, treatments as any);
        expect(insights).toContainEqual(expect.objectContaining({
...
*   **Code**:
    ...
    interface Treatment { date: number; carbs?: number; insulin?: number; }

    export function generateClusterInsights(cluster: GlycemicCluster, treatments: Treatment[]): Insight[] {
      const insights: Insight[] = [];
      ...
      cluster.events.forEach(event => {
        ...
        const relevant = treatments.filter(t => t.date >= lookback && t.date <= eventTime);
...
*   **Code Update**:
    ...
    const updateCall = mockPrismaUpdate.mock.calls.find((c: any) => c[0].data.status === 'COMPLETE');
